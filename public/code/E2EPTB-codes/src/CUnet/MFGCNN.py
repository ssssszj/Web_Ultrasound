...
class MFGCNN(nn.Module):
    def __init__(self):
        ...
        self.cUnet = ClassificationUnet(input_nc=3, output_nc=1, ngf=16)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc,
        ...
        if outermost:
            ...
            if use_dropout:
                model = down + [nn.Dropout(0.1)] + \
                    [downpool] + [submodule] + up
            else:
                model = down + [submodule] + up
...
class ClassificationUnet(nn.Module):
    def __init__(self, input_nc=1, output_nc=1, ngf=16,
        ...
        unet_block = UnetSkipConnectionBlock(
            ngf, ngf * 2, unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        unet_block = UnetSkipConnectionBlock(
